...
class GLTFMeshoptCompression {
  ...
  loadBufferView(index) {
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      return Promise.all([buffer, decoder.ready]).then(function (
        res
      ) {
...
class GLTFParser {
  ...
  loadAccessor(accessorIndex) {
    ...
    return Promise.all(pendingBufferViews).then(function (
      bufferViews
    ) {
      ...
      sourceURI = parser
        ...
        .then(function (bufferView) {
          if (source.mimeType === 'image/png') {
            ...
            hasAlpha =
              colorType === 6 || colorType === 4 || colorType === 3
          }
  ...
  loadAnimation(animationIndex) {
    ...
    for (let i = 0, il = animationDef.channels.length; i < il; i++) {
      ...
      pendingInputAccessors.push(
        this.getDependency('accessor', input)
      )
    ...
    return Promise.all([
      ...
      for (let i = 0, il = nodes.length; i < il; i++) {
        ...
        if (
          PATH_PROPERTIES[target.path] === PATH_PROPERTIES.weights
        ) {
      ...
      .then(function (mesh) {
        ...
        if (nodeDef.weights !== undefined) {
          node.traverse(function (o) {
            ...
            for (
              let i = 0, il = nodeDef.weights.length;
              i < il;
              i++
            ) {
